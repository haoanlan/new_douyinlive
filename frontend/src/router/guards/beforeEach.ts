...
import { fetchGetUserInfo } from '@/api/auth-douyin'
...
function isUnauthorizedError(error: unknown): boolean {
  // 模板 http 包装的 HttpError（code 401）
  if (isHttpError(error) && error.code === ApiStatus.unauthorized) return true
  // douyin-http 原生 axios 错误（HTTP 401）
  if (
    error &&
    typeof error === 'object' &&
    'response' in error &&
