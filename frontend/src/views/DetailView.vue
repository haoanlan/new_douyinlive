  ...
  <div>
    ...
    <div v-if="data && data.session.is_live" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
      ...
      <button class="btn btn-ghost btn-sm" @click="manualRefresh" :disabled="refreshing" style="font-size:12px;padding:4px 10px;display:flex;align-items:center;gap:4px;min-width:72px;justify-content:center">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="12" height="12" :style="{ animation: refreshing ? 'spin .6s linear infinite' : '' }">
          <path d="M23 4v6h-6"/>
                    ...
                    <template v-else>
                      ...
                      >
                        ...
                        <div style="flex:1;min-width:0;overflow:hidden">
                          ...
                          <div style="font-size:12px;color:var(--text-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap" :title="d.content">{{ d.content }}</div>
                        </div>
                      </div>
                    </template>
                  </div>
