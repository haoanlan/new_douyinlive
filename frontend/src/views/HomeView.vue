          ...
          <div style="overflow-x:auto">
            <table class="session-table">
              ...
              <tbody id="sessionTbody">
                <tr v-for="s in sessions" :key="s.id" :data-id="s.id" :data-start="s.started_at || ''" @click="viewDetail(s.id)">
...
function actionLabel(type: string) { return { gift: '礼物', danmaku: '弹幕', member: '进场' }[type] || type }
...
async function viewDetail(sessionId: number, fromPopState = false) {
  ...
  try {
    ...
    // Load danmaku data
    try {
      const dmData = await fetch(`${API}/api/sessions/${sessionId}/danmaku?limit=99999`).then(r => r.json())
      const raw = dmData.data || dmData || []
      _danmaku.value = raw.map((d: any) => ({
        ...d,
        timestamp: d.timestamp || d.create_time,
...
function clearDateFilter() {
  dpData.from = null
  dpData.to = null
  document.querySelectorAll('.session-table tbody tr').forEach(tr => (tr as HTMLElement).style.display = '')
}
...
function filterByDate() {
  ...
  document.querySelectorAll('.session-table tbody tr').forEach(tr => {
    const start = (tr as HTMLElement).dataset.start
    if (!start) return
...
function fmtSessionTime(t: any): string {
  if (!t) return ''
  let date: Date
...
function avatarHtml(url: string, name: string, size?: number): string {
...
function avatarHtml52(url: string, name: string): string {
