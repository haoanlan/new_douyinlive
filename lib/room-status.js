 ...
 *
 * 三级数据源，按可靠性从高到低：
 *   0) memory —— worker 与仪表盘同进程（默认），直接读内存：实时、精确、零延迟
 *   1) socket —— worker 仍作为独立进程运行时，走 monitor.sock（仅独立模式）
 *   2) log    —— 以上都不可用时，解析 logs/daemon.log 兜底（受限环境建不出管道时用）
 *
 * 为什么要抽出来：
 ...
 *   ageMs（数据年龄，毫秒）, stale（是否已过期）, source（memory | socket | log）
 */
const fs = require('fs');
...
/** 嵌入模式探测：worker 就在本进程里时返回控制模块，否则返回 null */
function getEmbeddedWorker() {
  try {
    const wc = require('./worker-control.js');
    return wc.isWorkerRunning() ? wc : null;
  } catch {
    return null;
  }
}
...
async function getRoomStates({ dbInstance, dataDir }) {
  ...
  // 0) 嵌入模式（默认）：worker 就在本进程里，直接读内存 —— 实时、精确、零延迟
  const embedded = getEmbeddedWorker();
  if (embedded) {
    const memRooms = embedded.getRoomStates();
    if (memRooms.length) {
      return {
        source: 'memory',
        stale: false,
        ageMs: 0,
        rooms: memRooms.map((r) => ({
          ...r,
          // 与 socket 分支保持一致：优先用 DB 里的主播名（页面按名字二次匹配时要用）
          name: nameToId[String(r.roomId)] || r.name || '',
        })),
      };
    }
  }

  // 1) 控制通道（worker 作为独立进程运行时才有）
