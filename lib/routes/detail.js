...
module.exports = async function(pathname, query, req, res, ctx) {
  ...
  if (pathname.startsWith('/api/sessions/') && pathname.endsWith('/detail')) {
    ...
    // 弹幕（用于展示和搜索）
    const latestDanmaku = dbInstance.prepare(`
    ...
    const danmakuCount = dbInstance.prepare(
      'SELECT COUNT(*) as cnt FROM danmaku WHERE session_id = ?'
    ).get(sid).cnt;
    ...
    const summary = {
      ...
      total_danmaku: danmakuCount,
      danmaku_count: danmakuCount,
      user_count: (() => {
        const giftUsers = new Set(dedupedGifts.map(g => g.user_sec_uid).filter(Boolean));
        const danmakuUsers = dbInstance.prepare(
          'SELECT COUNT(DISTINCT user_sec_uid) as cnt FROM danmaku WHERE session_id = ? AND user_sec_uid IS NOT NULL'
        ).get(sid);
        return giftUsers.size + (danmakuUsers?.cnt || 0);
      })(),
