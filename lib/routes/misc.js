...
module.exports = async function(pathname, query, req, res, ctx) {
  ...
  if (pathname === '/api/summary') {
    ...
    `).get();
    // 礼物和钻石：用 SQL 聚合近似（去重后 total_diamonds 有微小误差，但避免全量加载11万条）
    const giftStats = dbInstance.prepare(`
      SELECT
        SUM(repeat_count) as total_gifts,
        SUM(total_diamonds) as total_diamonds
      FROM gifts
    `).get();
    stats.total_gifts = giftStats.total_gifts || 0;
    stats.total_diamonds = giftStats.total_diamonds || 0;
