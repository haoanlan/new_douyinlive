...
module.exports = async function (pathname, query, req, res, ctx) {
  ...
  if (pathname === '/api/overview') {
    ...
    const rawGifts = d.prepare(`
      SELECT id, nickname, avatar, user_display_id, gift_name, user_sec_uid, to_user_sec_uid,
        to_user_display_id, to_nickname, repeat_count, total_diamonds, combo_count, repeat_end, create_time, icon
    ...
    for (const g of deduped) {
      const k = g.gift_name;
      if (!giftMap.has(k)) giftMap.set(k, { name: k, icon: g.icon || '', count: 0, diamonds: 0 });
    ...
    for (const g of deduped) {
      ...
      if (!userMap.has(k)) userMap.set(k, { nickname: g.nickname, avatar: g.avatar || '', sec_uid: g.user_sec_uid || '', diamonds: 0, count: 0 });
      const e = userMap.get(k);
      e.diamonds += g.total_diamonds || 0;
      e.count += g.repeat_count || 1;
      if (g.nickname) e.nickname = g.nickname;
      if (g.avatar) e.avatar = g.avatar;
    }
