  ...
  if (pathname === '/api/rooms/lookup' && req.method === 'GET') {
    ...
    try {
      if (/^[A-Za-z0-9]{5,30}$/.test(input)) {
        ...
        // 1) 代理查状态（离线房间也能拿到 title / has_room）
        let hasRoom = false, isLive = false, roomStatus = 'unknown';
        let roomTitle = '', nickname = '', avatar = '', nameSource = 'none';

        const batch = await proxyJson('/api/v1/rooms/status:batch', { method: 'POST', body: { live_ids: [input] } });
        const item = batch?.data?.items?.[0];
        if (item) {
          hasRoom = item.has_room === true;
          isLive = item.is_live === true;
          roomStatus = item.status || 'unknown';   // online | offline | unknown
          roomTitle = item.title || '';
        }

        // 2) 开播中 → 代理能给出主播昵称与头像
        if (isLive) {
          const anchor = await proxyJson(`/api/v1/rooms/${encodeURIComponent(input)}/anchor`);
          const a = anchor?.data?.anchor;
