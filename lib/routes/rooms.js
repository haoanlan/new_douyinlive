...
module.exports = async function(pathname, query, req, res, ctx) {
  ...
  if (pathname === '/api/rooms/add' && req.method === 'POST') {
    ...
    // 把主播名/头像落库。
    // 之前只在填了 name 时才写、而且完全不存头像 → 添加后卡片显示房间号且没有头像，
    // 尽管添加前的预览明明已经查到了昵称和头像（前端会把预览结果一并带过来）。
    if (result.ok) {
      ...
      if (finalName || finalAvatar) {
        try {
          ...
          if (existing) {
            // 已有记录：只覆盖非空字段，避免把之前解析好的名字/头像清掉
            dbInstance.prepare(`
              UPDATE streamers
                 SET name = COALESCE(NULLIF(?, ''), name),
                     avatar = COALESCE(NULLIF(?, ''), avatar)
               WHERE room_id = ?
            `).run(finalName, finalAvatar, room_id);
          } else {
            dbInstance.prepare('INSERT INTO streamers (name, room_id, avatar) VALUES (?, ?, ?)')
              .run(finalName || room_id, room_id, finalAvatar || null);
          }
        } catch (e) { console.error('[add] 写入 streamers 失败:', e.message); }
      }
    }
    return sendJSON(res, result, workerControl.controlStatus(result));
  }
