...
module.exports = async function(pathname, query, req, res, ctx) {
  ...
  if (pathname === '/api/sessions') {
    ...
    // 对直播中（end_time IS NULL）的场次，实时计算 agg 字段
    for (const row of rows) {
      if (row.end_time == null) {
        try {
          const rawGifts = dbInstance.prepare('SELECT * FROM gifts WHERE session_id = ? ORDER BY id').all(row.id);
          if (rawGifts.length > 0) {
            const deduped = comboDedupGifts(rawGifts);
            row.agg_gifts = deduped.reduce((s, g) => s + (g.repeat_count || 1), 0);
            row.agg_diamonds = deduped.reduce((s, g) => s + (g.total_diamonds || 0), 0);
            const dmRow = dbInstance.prepare('SELECT COUNT(*) as cnt FROM danmaku WHERE session_id = ?').get(row.id);
            row.agg_danmaku = dmRow.cnt;
          }
        } catch (e) { /* ignore */ }
      }
    }
  ...
  if (pathname.startsWith('/api/hosts/') && pathname.endsWith('/sessions')) {
    ...
    const rows = dbInstance.prepare(
      'SELECT s.*, st.avatar as streamer_avatar, st.name as streamer_name FROM sessions s LEFT JOIN streamers st ON s.streamer_id = st.id WHERE s.streamer_id = ? ORDER BY s.start_time DESC'
    ).all(streamer.id);
    // 对直播中（end_time IS NULL）的场次，实时计算 agg 字段
    for (const r of rows) {
      if (r.end_time == null) {
        try {
          const rawGifts = dbInstance.prepare('SELECT * FROM gifts WHERE session_id = ? ORDER BY id').all(r.id);
          if (rawGifts.length > 0) {
            const deduped = comboDedupGifts(rawGifts);
            r.agg_gifts = deduped.reduce((s, g) => s + (g.repeat_count || 1), 0);
            r.agg_diamonds = deduped.reduce((s, g) => s + (g.total_diamonds || 0), 0);
            const dmRow = dbInstance.prepare('SELECT COUNT(*) as cnt FROM danmaku WHERE session_id = ?').get(r.id);
            r.agg_danmaku = dmRow.cnt;
          }
        } catch (e) { /* ignore */ }
      }
    }
    return sendJSON(res, rows.map(r => ({
      id: r.id,
      title: `场次 #${r.id}`,
      streamer_avatar: r.streamer_avatar || '',
