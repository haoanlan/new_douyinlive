module.exports = async function(pathname, query, req, res, ctx) {
  ...
  if (pathname === '/api/anonymous-lookup') {
    ...
    if (pendingGiftRows.length) {
      ...
      let giftDetails;
      if (uid) {
        giftDetails = dbInstance.prepare(
          `SELECT user_sec_uid, session_id, create_time, gift_name, repeat_count, to_nickname FROM gifts WHERE user_sec_uid = ? AND (session_id, create_time) IN (${ph})`
        ).all(uid, ...params);
      } else {
        giftDetails = [];
      ...
      for (const g of giftDetails) {
        giftIdx[(g.user_sec_uid || g.nickname || '') + '\x00' + g.session_id + '\x00' + g.create_time] = g;
      }
      for (const u of Object.values(userMap)) {
        if (!u._pendingGiftDetails) continue;
        for (const r of u._pendingGiftDetails) {
          const g = giftIdx[(uid || r.nickname || '') + '\x00' + r.session_id + '\x00' + r.create_time];
